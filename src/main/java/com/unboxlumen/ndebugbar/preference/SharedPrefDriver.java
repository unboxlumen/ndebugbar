...
public class SharedPrefDriver {
    ...
    public SharedPrefDriver(Context context) {
        ...
        if (VERSION.SDK_INT >= 24) {
            this.DPStorageContext = context.createDeviceProtectedStorageContext();
        }
    ...
    private static void fillEditor(SharedPreferences.Editor editor, String key, String newValue, Object existingValue) {
    ...
    public List<File> getSharedPrefDescs(IProvider provider) {
        List<File> files = provider.getSharedPrefFiles(this.context);
        return files;
    }

    public Map<String, String> getSharedPrefContent(File file) {
        SharedPreferences prefs = this.getSharedPreferences(file);
        Map<String, String> transformed = new HashMap();

        for (Map.Entry<String, ?> entry : prefs.getAll().entrySet()) {
            transformed.put((String) entry.getKey(), convert2String(entry.getValue()));
        }
    ...
    private SharedPreferences getSharedPreferences(File file) {
        ...
        if (file.getPath().contains("/user_de/")) {
            sp = this.DPStorageContext.getSharedPreferences(noSuffixName, 0);
        } else {
            sp = this.context.getSharedPreferences(noSuffixName, 0);
        }
