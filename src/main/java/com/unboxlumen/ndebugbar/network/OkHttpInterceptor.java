...
public class OkHttpInterceptor implements Interceptor {
    public static final long MAX_SIZE_BODY = 2097152L;
    ...
    private static String requestBodyAsStr(Request request) {
        ...
        if (requestBody == null) {
            ...
            if (contentType == null || TextUtils.isEmpty(contentType.toString()) || !contentType.toString().contains("form-data") && !contentType.toString().contains("octet-stream")) {
                ...
                if (!isPlaintext(buffer)) {
                    ...
                    try {
                        if (requestBody.contentLength() > 2097152L) {
                            return "(binary " + requestBody.contentLength() + "-byte body omitted)";
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
    ...
    private static String responseBodyAsStr(Response response) {
        ...
        if (responseBody != null && HttpHeaders.hasBody(response)) {
            ...
            if (responseBody.contentLength() > 2097152L) {
                return "(binary " + responseBody.contentLength() + "-byte body omitted)";
            } else {
                ...
                try {
                    return sourceToStrInternal(response.peekBody(Long.MAX_VALUE).source(), gzip, responseBody.contentType());
                } catch (IOException e) {
                    e.printStackTrace();
                    return null;
                }
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        try {
            ...
            if (Config.isNetLogEnable() && id >= 0L) {
                this.markFailed(id, Utils.collectThrow(var13));
                this.notifyEnd(id);
            }
